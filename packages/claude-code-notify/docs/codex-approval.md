...
- `codex-watch` 保留为 app-server-scoped / debugging 模式，不作为默认无感方案。
...
MCP sidecar 擅长的是“Codex 这次是从哪个 terminal 启动的”，而不是“后面某个 approval 事件属于哪个 session / thread / turn”。后者当前仍然要靠 rollout / `codex-tui.log` / app-server 这类语义通道补齐。
...
在这个约束下，`codex app-server` 更适合“你自己就是 Codex 的宿主 / 集成方”的窄场景，而不是 stock Codex TUI 会话的默认全局 approval 旁听器。
...
**为什么要额外做 `autostart` 命令？**

`codex-session-watch` 是长期驻留的后台观察者。主路径已经变成“由 sidecar 在第一条 Codex session 启动时兜底拉起”；但仍然保留 `autostart`，给“不想依赖 sidecar”“希望 watcher 在第一条 session 之前就已经在线”的用户一个显式选项。

**为什么自启动用 HKCU Run + `wscript.exe` 隐藏启动？**

是否开机自启应该是用户显式决定的行为，而不是 npm install 的隐式副作用。当前实现通过 HKCU 的 `Run` 注册表项拉起 watcher，并借助 `scripts/start-hidden.vbs` 隐藏窗口。

**为什么 session watcher 需要单实例锁？**

一旦支持登录自启动，就很容易出现“后台已经有一份 watcher，用户又手工执行了一次 `codex-session-watch`”的情况。没有单实例保护时，两份 watcher 会同时扫描同一批 rollout / TUI 日志，最终发出重复 Toast。
...
后台 watcher 自己拉起的 `codex app-server` 只能稳定观察到它自己那条连接里的 thread 状态，不能作为“其他 Codex 会话 approval 的全局观察者”。这条验证的完整过程见 [`history/codex-approval-notification-session-2026-03-18.md`](./history/codex-approval-notification-session-2026-03-18.md)。
...
单 `cwd` 模式的过滤只显式出现在 `thread/list` bootstrap 请求上；后续实时 `thread/status/changed` 通知本身只带 `threadId` 和 `status`，不带 `cwd`。因此它应被理解为：

- 对它自己那条 app-server 连接的状态观察有效
- 但不是“严格按项目隔离、严格全局正确”的 approval 观察器
- 日常使用的 approval 主路径仍然是 `codex-session-watch`
