...
这一页记录 Windows 平台上的运行时细节、wrapper 行为、通知实现和已知问题。带日期的演进过程统一放到 [`history/`](./history/)。
...
**最终方案：** `VSCODE_GIT_IPC_HANDLE` 命名管道。通过 Win32 API `GetNamedPipeServerProcessId` 获取 pipe 服务端 PID，再向上一层定位到主窗口。
...
**最终方案：** VBScript wrapper。由 `wscript.exe` 调用 VBScript，再由 VBScript 调用 PowerShell，利用 `shell.Run command, 0, False` 完全隐藏控制台。
